...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅ KING KAVIYA😍 working!')
        );
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (conn.user.jid === '@s.whatsapp.net') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*😍✅ දෝෂ විශ්ලේෂණය [KING KAVIYA] 🧟‍♀️*' + 
                                        '\n========== ```දෝෂ නිරාකරණය කර ඇත!``` ==========' +
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(conn.user.jid, '*😍✅ දෝෂ විශ්ලේෂනය [KING KAVIYA] 🧟‍♀️*' + 
                                        '\n========== ```දෝෂ නිරාකරණය කර ඇත!``` ==========' +
                                        '\n\n*ප්‍රධාන දෝශය:* _භාවිත කලහැක්කෙ URL පමණි_' +
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ *KING KAVIYA ERROR FOUND!* ~______~*' +
                                    '\n\n*🧞‍♂️ ' + error + '*\n\n```*මෙය ඔබගෙ LOG අංකයවෙ  මෙහි විධානයන් භාවිතා කිරිමෙන් වරකින්න*\n\n *වෙනත් ඔනැම චැට් එකකදි ඔබට විධානයන් භාවිතා කළ හැකිය...*\n\n\n\n *මෙය ＫＩＮＧ ＫＡＶＩＹＡ වෙ*\n\n\ *ⓀⒾⓃⒼ ⓀⒶⓋⒾⓎⒶ ⒷⓄⓉ බාවිතා කිරිම ගැන අපට ඔබ ගැන සතුටුයි*\n\n\n⚜️ *ප්‍රධාන  විධානය*👉🏻 *.king* \n\n\n Support ✑https://chat.whatsapp.com/HkaHOCPgMdJF5eUlRPxTkP*``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
